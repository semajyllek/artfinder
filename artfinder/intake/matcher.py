...
def strip_diacritics(text: str) -> str:
    ...
    (e.g. 'Cézanne' -> 'Cezanne', 'Münch' -> 'Munch'), so datasets that
    differ only in accent marks/unicode normalization still match."""
    if not text: return ""
...
def replace_delimiters(text: str) -> str:
    """Converts kebab-case hyphens, underscores, slashes, and ampersands to clean spaces/words."""
    if not text: return ""
...
def collapse_whitespace(text: str) -> str:
    """Collapses runs of whitespace (left behind by punctuation/delimiter removal) to single spaces."""
    if not text: return ""
    return re.sub(r'\s+', ' ', text).strip()


def strip_leading_article(text: str) -> str:
    """Drops a leading 'the'/'a'/'an' token, e.g. 'the starry night' -> 'starry night'."""
    tokens = text.split()
...
def strip_trailing_article(text: str) -> str:
    """Drops a trailing 'the'/'a'/'an' token (catalog-style 'starry night the' -> 'starry night')."""
    tokens = text.split()
    if len(tokens) > 1 and tokens[-1] in _ARTICLES:
        return ' '.join(tokens[:-1])
    return text
...
def normalize_text_structure(text: str) -> str:
    ...
    through cleaning cells and flattening cases. Used as the shared base
    normalizer for both artist names and artwork titles.
    """
    if not text or not isinstance(text, str):
        return ""

    cleaned = strip_diacritics(text)
    cleaned = strip_parentheticals(cleaned)
...
def normalize_title_structure(text: str) -> str:
    """
    Like normalize_text_structure, but additionally normalizes a leading
    or trailing definite/indefinite article so 'The Starry Night' and
    'Starry Night, The' (a common library-catalog form) both reduce to
    'starry night'.
    """
    cleaned = normalize_text_structure(text)
    cleaned = strip_trailing_article(cleaned)
    cleaned = strip_leading_article(cleaned)
    return cleaned
...
def match_artwork_titles(stream_title: str, target_title: str, fuzzy_threshold: int = 85) -> bool:
    ...
    alignment to tolerate minor spelling typos, missing articles, or
    accent/punctuation differences across datasets.
    """
    clean_stream = normalize_title_structure(stream_title)
    clean_target = normalize_title_structure(target_title)
